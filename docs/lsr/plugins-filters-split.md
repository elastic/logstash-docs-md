...
An example use case of this filter is for taking output from the [exec input plugin](plugins-inputs-exec.md) which emits one event for the whole output of a command and splitting that output by newline - making each line an event.
