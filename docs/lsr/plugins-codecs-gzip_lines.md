...
For other versions, see the [Versioned plugin docs](/vpr/codec-gzip_lines-index.md).

## Installation [_installation_69]

For plugins not bundled by default, it is easy to install by running `bin/logstash-plugin install logstash-codec-gzip_lines`. See [Working with plugins](logstash://reference/working-with-plugins.md) for more details.
